...
for conv in corpus.iter_conversations():
    ...
    for i in range(len(utts) - 1):
        prompt = clean_text(utts[i].text)
        reply  = clean_text(utts[i+1].text)
        if prompt and reply:
...
# --------- CHECKPOINT: LOAD WEIGHTS IF AVAILABLE ---------
MODEL_PATH = "chatbot_lstm_attn.pth"
...
while True:
    ...
    if prompt.strip().lower() in {"quit", "exit"}:
        break
    reply = generate_reply(model, prompt, max_len=40, temp=0.8)
    print(f"Bot: {reply}")
